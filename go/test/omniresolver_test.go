...
import (
	...
	omni "github.com/voxgig-sdk/yugi-limit-regulation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YugiLimitRegulationError); is {
				out := map[string]any{
					"name":    "YugiLimitRegulationError",
...
func omniCtx(arg any, client *sdk.YugiLimitRegulationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YugiLimitRegulationSDK); is {
			client = live
