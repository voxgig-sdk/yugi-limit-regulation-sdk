...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("YUGILIMITREGULATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("YUGILIMITREGULATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("YUGILIMITREGULATION_TEST_EXPLAIN"); explain != "" {
