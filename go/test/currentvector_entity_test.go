...
func TestCurrentvectorEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		currentvectorRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.currentvector")))
...
func currentvectorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YUGI_LIMIT_REGULATION_TEST_CURRENTVECTOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YUGI_LIMIT_REGULATION_TEST_CURRENTVECTOR_ENTID"])
