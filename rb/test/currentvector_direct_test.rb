...
class CurrentvectorDirectTest < Minitest::Test
  def test_direct_list_currentvector
    setup = currentvector_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-currentvector", setup[:live] ? "live" : "unit")
    if _should_skip
...
def currentvector_direct_setup(mockres)
