...
class CurrentvectorEntity
  ...
  def make
    opts = @_entopts.dup
    CurrentvectorEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YugiLimitRegulationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Currentvector, Hash] the current Currentvector data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = YugiLimitRegulationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Currentvector fields)
  def match_get
  ...
  #
  # @param reqmatch [CurrentvectorListMatch, Hash, nil] match filter (any subset of
  #   Currentvector fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Currentvector>, Array] the matching Currentvector items; raises YugiLimitRegulationError on failure
