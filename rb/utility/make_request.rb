...
module YugiLimitRegulationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YugiLimitRegulationResponse.new({})
    ...
    if fetch_err
      ...
      response = YugiLimitRegulationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YugiLimitRegulationResponse.new(fetched)
    else
