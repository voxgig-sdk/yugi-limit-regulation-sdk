...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.YUGILIMITREGULATION_TEST_LIVE ||
    'TRUE' === process.env.YUGILIMITREGULATION_TEST_OVERRIDE
  ) {
  ...
  m.YUGILIMITREGULATION_TEST_EXPLAIN = process.env.YUGILIMITREGULATION_TEST_EXPLAIN || m.YUGILIMITREGULATION_TEST_EXPLAIN
