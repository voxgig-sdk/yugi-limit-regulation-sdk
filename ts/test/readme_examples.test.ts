...
//     (\`new YugiLimitRegulationSDK(...)\` / \`YugiLimitRegulationSDK.test(...)\` become
//     \`YugiLimitRegulationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YugiLimitRegulationSDK'
...
const TEST_SEED = {"entity":{"currentvector":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YugiLimitRegulationSDK(...)\` and any
// \`YugiLimitRegulationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YugiLimitRegulationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YugiLimitRegulationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
