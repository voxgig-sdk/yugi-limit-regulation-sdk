...
//    only - a typed YugiLimitRegulationContext there would make every ctx
...
final class YugiLimitRegulationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new YugiLimitRegulationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new YugiLimitRegulationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new YugiLimitRegulationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new YugiLimitRegulationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new YugiLimitRegulationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?YugiLimitRegulationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new YugiLimitRegulationError(is_string($code) ? $code : '', $msg);
    }
