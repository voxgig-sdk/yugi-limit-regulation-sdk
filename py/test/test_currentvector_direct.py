...
class TestCurrentvectorDirect:

    def test_should_direct_list_currentvector(self):
        setup = _currentvector_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-currentvector", "live" if setup["live"] else "unit")
        if _skip:
...
def _currentvector_direct_setup(mockres):
