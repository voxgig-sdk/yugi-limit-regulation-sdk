# YugiLimitRegulation SDK test runner
...
class YugiLimitRegulationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YugiLimitRegulationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YugiLimitRegulationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YugiLimitRegulationTestRunner.getenv("YUGI_LIMIT_REGULATION_TEST_LIVE")
        override = YugiLimitRegulationTestRunner.getenv("YUGI_LIMIT_REGULATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YugiLimitRegulationTestRunner.getenv(key)
        ...
        explain = YugiLimitRegulationTestRunner.getenv("YUGI_LIMIT_REGULATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if YugiLimitRegulationTestRunner._test_control is not None:
            return YugiLimitRegulationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YugiLimitRegulationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YugiLimitRegulationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YugiLimitRegulationTestRunner.load_test_control()
...
def load_env_local():
    YugiLimitRegulationTestRunner.load_env_local()


def env_override(m):
    return YugiLimitRegulationTestRunner.env_override(m)


def entity_data(v):
    return YugiLimitRegulationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YugiLimitRegulationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YugiLimitRegulationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YugiLimitRegulationTestRunner.load_test_control()


def live_client_options():
    return YugiLimitRegulationTestRunner.live_client_options()


def live_delay_ms():
    return YugiLimitRegulationTestRunner.live_delay_ms()
