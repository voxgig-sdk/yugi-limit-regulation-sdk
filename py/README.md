...
The Python SDK for the YugiLimitRegulation API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Currentvector(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.YugiLimitRegulation(None).load(
    {"id": "test01"}, None
)
