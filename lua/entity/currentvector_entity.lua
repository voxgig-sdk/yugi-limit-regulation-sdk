...
local CurrentvectorEntity = {}
CurrentvectorEntity.__index = CurrentvectorEntity


function CurrentvectorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CurrentvectorEntity)
  self._name = "currentvector"
  self._client = client
...
function CurrentvectorEntity:get_name()
  return self._name
end


function CurrentvectorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CurrentvectorEntity.new(self._client, opts)
end
...
function CurrentvectorEntity:mark_deleted()
  self._deleted = true
end


function CurrentvectorEntity:deleted()
  return true == self._deleted
end


function CurrentvectorEntity:data_set(args)
  if args ~= nil then
...
function CurrentvectorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CurrentvectorEntity:match_set(args)
  if args ~= nil then
...
function CurrentvectorEntity:match_get()
...
function CurrentvectorEntity:stream(action, args, callopts)
...
function CurrentvectorEntity:_run_op(ctx, post_done)
  local utility = self._utility
