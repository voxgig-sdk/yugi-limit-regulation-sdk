...
The Lua SDK for the YugiLimitRegulation API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Currentvector(nil):list(nil, nil)
...
local result, err = client:YugiLimitRegulation(nil):load(
  { id = "test01" }, nil
)
