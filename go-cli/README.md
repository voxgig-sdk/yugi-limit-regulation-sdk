...
boru-driven command-line client **and** interactive REPL for the YugiLimitRegulation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yugi-limit-regulation-cli)
make build

# 2. See usage (words, entities, env vars)
./yugi-limit-regulation-cli --help
...
YUGI_LIMIT_REGULATION_BASE=https://api.example.com ./yugi-limit-regulation-cli list currentvector

# 6. No arguments -> interactive REPL
./yugi-limit-regulation-cli
yugi-limit-regulation> list currentvector
yugi-limit-regulation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yugi-limit-regulation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yugi-limit-regulation-cli list currentvector
   ```
...
Run with no arguments to open a REPL (prompt `yugi-limit-regulation>`). Each line is
evaluated as its own boru expression:

```text
$ ./yugi-limit-regulation-cli
yugi-limit-regulation> list currentvector
yugi-limit-regulation> /help
yugi-limit-regulation> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `currentvector`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yugi-limit-regulation-cli`. |
...
(`./yugi-limit-regulation-cli <expr>`) and interactively (the REPL), and expressions compose the
